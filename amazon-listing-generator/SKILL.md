...
## Product Visual Fidelity Rules

- 产品外观、结构、颜色、纹理、配件、包装、收纳盒、标签、Logo、尺寸标识和数量必须 100% 基于用户提供的实物图片、原图或明确资料。
- 生成主图、附图、A+、视频脚本、作图 Brief 或 AI 作图提示词时，不允许 AI 重新设计产品、改造结构、改变颜色、增加不存在的零件、删除真实配件、改变包装样式或虚构使用形态。
- 可以优化画面构图、光线、清晰度、背景、标注层级和信息排版，但产品本体和包装必须保持真实一致。
- 如果图片或资料中看不清外观、结构、包装或配件，必须标注“需要用户确认”，不能猜测补全。
- 对设计师或 AI 作图工具的输出要求必须写明：Use the provided product photo as the source of truth; keep product appearance, structure, color, accessories, packaging, and quantity exactly the same; do not redesign the product.
...
- 产品外观、结构、包装与用户提供的真实资料必须一致，不允许为了美观或差异化重新设计产品。
- 前台表述、后端属性、视觉 Brief 和 Rufus 可回答信息必须一致。
